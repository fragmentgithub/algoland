...
js/speech.js    speak()(ja-JP音声を自動選択: Nanami>natural>Google>Kyoko)、🔈ボタン、ミュート。
                Android対策を含む(簡略化禁止): cancel()直後のspeak()は捨てられるため80ms遅延、
                voices遅延ロードのため発話時に再取得、バックグラウンド後のpaused固着対策のresume()、
                発話オブジェクトのGC回収対策の参照保持。診断は⚙️の「こえをテスト」
...
- **装飾の常時ループアニメーション全面禁止(恒久決定)**: 雲の横流し→減速4回→上下ふわふわ→
  タイトル文字の波打ちまで、計4回「速すぎる」と指摘された。個々を遅くしても複数要素+時間差で
  「常にどこかが動いている」状態になり忙しく見えるため、減速では解決しない。
  雲・タイトル文字・マスコット・おともだち欄はすべて静止。動きは「意味のあるフィードバック」
  (wobble/arrive/hintpulse/?セルのpulse/お祝いモーダル)だけに限定する。復活させないこと。
  CSS更新時は index.html の ?v=・.app-version・sw.js の CACHE を3点セットで上げる
